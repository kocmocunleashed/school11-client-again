...
export function AboutContent() {
  ...
  return <main id="main-content" className="inner-page shell">
    ...
    <section className="story-grid"><div><p className="overline">Манай сургууль</p><h2>{copy.story_title}</h2></div><div><p>{copy.story_description}</p></div></section>
    ...
    <section className="history-section"><header><p className="overline">Түүхэн замнал</p><h2>Үе үеийн хөгжил</h2></header><ol>{achievements.filter(year => year.is_milestone).toSorted((a,b) => a.year-b.year).map(year => <li key={year.id}><time>{year.year}</time><div><h3>{year.highlight_mn}</h3><p>{year.description_mn}</p></div></li>)}</ol></section>
    <TeachersGallery />
