...
export function AboutContent() {
  ...
  return <main id="main-content" className="inner-page shell mx-auto w-[var(--shell)]">
    ...
    <section className="principal-section" id="principal"><header><p className="school-eyebrow">Мэндчилгээ</p><h2>Захирлын үг</h2><span className="principal-mark" aria-hidden="true">“</span></header><div>{copy.principal_message ? <><p className="principal-message text-[clamp(17px,_1.3vw,_22px)] whitespace-pre-line">{copy.principal_message}</p>{copy.principal_name && <p className="principal-signature"><strong>{copy.principal_name}</strong><span>Сургуулийн захирал</span></p>}</> : <p className="empty-state">Захирлын мэндчилгээ удахгүй нийтлэгдэнэ.</p>}</div></section>
    <section className="story-grid" id="school-history"><div><p className="school-eyebrow">Сургуулийн түүх</p><h2>{copy.story_title}</h2></div><div><p>{copy.story_description}</p></div></section>
    ...
    <section className="values-section"><header><p className="school-eyebrow">Бидний баримтлах зарчим</p><h2>Өдөр тутмын дөрвөн үнэт зүйл</h2></header><div className="value-grid">{values.map(([Icon, title, text]) => <article key={title}><Icon /><h3>{title}</h3><p>{text}</p></article>)}</div></section>
    <section className="history-section" id="school-journey"><header><p className="school-eyebrow">Түүхэн замнал</p><h2>Он жилээр бүтсэн замнал</h2></header><ol>{achievements.toSorted((a,b) => a.year-b.year).map(year => <li key={year.id}><time>{year.year}</time><div><h3>{year.highlight_mn}</h3><p>{year.description_mn}</p></div></li>)}</ol></section>
    <TeachersGallery />
    <section className="contact-section" id="contact"><header><p className="school-eyebrow">Холбоо барих</p><h2>Сургуультай холбогдох</h2></header><div className="contact-grid"><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${settings.address_mn}, ${settings.city}`)}`} target="_blank" rel="noreferrer"><MapPin /><span><small>Хаяг</small><strong>{settings.address_mn}, {settings.city}</strong></span></a><a href={`tel:${settings.phone.replace(/\s/g, "")}`}><Phone /><span><small>Утас</small><strong>{settings.phone}</strong></span></a><a href={`mailto:${settings.email}`}><Mail /><span><small>И-мэйл</small><strong>{settings.email}</strong></span></a></div></section>
  </main>;
}
