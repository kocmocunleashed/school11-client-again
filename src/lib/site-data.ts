...
export const getSiteData = cache(async (): Promise<PublicSiteData> => {
  ...
  if (hall.error) throw new Error(`Hall of Fame CMS unavailable; check the redesign migration: ${hall.error.message}`);
  // Empty published collections are intentional. Never resurrect deleted/draft content as samples.
  return publishableData({ news, teachers, settings, achievements, courses, hallOfFame: hall.data || [], preview: false });
});
